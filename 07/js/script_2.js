      ...
      // どれが正解かはidを比較すればいいので特に設定しなくてよい（codeの比較でもいいがせっかく作ったのでそっちを使う）
        ...
        // 正解でないなら
          ...
          while(searching_1){
            ...
            for(let l = 0; l < j; l++){
              // question_data[i][1] から question_data[i][j]までに k が重複していないか確認と数値が重複していなくても問題のcodeが正解と同じ場合がありうるのでそちらで比較する
              if((question_data[i][l] === k) || (json_data[question_data[i][0]].code === json_data[k].code)){
                // 重複していたらループを抜ける
                searching_2 = false;
                console.log( 'l1:'+l );
                console.log( 'k1:'+k );
                console.log( 'question_data[i][l]1:'+question_data[i][l] );
                break;
              }
                console.log( 'l2:'+l );
                console.log( 'k2:'+k );
                console.log( 'question_data[i][l]2:'+question_data[i][l] );
            }
            // 整数kが他の選択肢と重複していないなら
            if(searching_2){
